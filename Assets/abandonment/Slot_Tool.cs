...
public class Slot_Tool : MonoBehaviour, IPointerClickHandler, IInventorySlotRightClickable
{
...
private ItemType itemType = ItemType.Tool;         // 타입 고정(도구)
...
private InventoryUI ownerUI;                       // 호출자(UI)
    ...
    public void Bind(ItemViewData v)            // 바인딩
    {
        ...
        if (nameText)  nameText.text  = string.IsNullOrEmpty(v.name) ? "Tool" : v.name;
    }
