...
public class Slot_Ranged : MonoBehaviour, IPointerClickHandler, IInventorySlotRightClickable
{
...
private ItemType itemType = ItemType.Ranged;       // 타입 고정(원거리)
...
private InventoryUI ownerUI;                       // 호출자(UI)
    ...
    public void Bind(ItemViewData v)            // 바인딩
    {
        ...
        if (nameText)  nameText.text  = string.IsNullOrEmpty(v.name) ? "Ranged" : v.name;
    }
